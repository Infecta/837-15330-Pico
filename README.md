...
| SN74LV1T34DBVR Logic Level Shifter | 1 | This is for the SK6812MINI-E LED data pins (Needs 5V logic rather than 3.3V logic). You can substitute this with any other level shifter provided it's pin compatible or modify the PCB.
...
| 330R | 1 | Needed for the logic level shifter. Followed datasheet. If changing level shifter check datasheet for info.
...
Peak for a single IR phase (2 IR Emitters) would be ~80mA and ~42mA respective too, which is well within the budget

## Build _*"guide"*_

This doc fully assumes you are comfortable with soldering SMD components.
Protip: Use the Interactive Html Bom plugin on KiCAD to figure out where all the components go.
